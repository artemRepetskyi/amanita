...
export default class Checkout {
 	...
 	renderItems(){
 		...
 		for (var prop in productItems) {
		  ...
		  if (productItems[prop].item !==  undefined && productItems[prop].item >= 1) {
				...
				<div class="c-cart-product">
					...
					<div class="c-cart-product__content">
						...
						<div class="c-counter js-c-counter">
						    ...
						    <input data-id="${productItems[prop].id}" type="text" class="c-counter__input input-Qty" value="${productItems[prop].item}">
						    <div class="c-counter__btn plus"></div>
						</div>
						<p class="text-5">${productItems[prop].price * productItems[prop].item} грн</p>
					</div>
				</div>
				<input type="hidden" name="${productItems[prop].id}" value="(${productItems[prop].title} | ${productItems[prop].item} штук | ${productItems[prop].price * productItems[prop].item} грн)">
			</li>`;
			html += template;
		  }
		}
